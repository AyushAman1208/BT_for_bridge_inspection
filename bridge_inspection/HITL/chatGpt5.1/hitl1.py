...
,
    ...
    def createBehaviorTree(self):
        ...
        preflight_parallel = py_trees.composites.Parallel(
            name=f"Preflight_Both",
            policy=py_trees.common.ParallelPolicy.SuccessOnAll()
            )
        preflight_parallel.add_children([
            py_trees.composites.Sequence(name=f"UAV1_Preflight", memory=True).add_children([]) or self.armDrone(droneId1),
        ...
        uav1_pre = py_trees.composites.Sequence(name=f"UAV1_PreflightSeq", memory=True)
        uav1_pre.add_children([self.armDrone(droneId1), self.offboard(droneId1), self.takeOff(droneId1)])
        uav2_pre = py_trees.composites.Sequence(name=f"UAV2_PreflightSeq", memory=True)
        ...
        root.add_children([preflight_parallel])
        ...
        for idx, waypoint in enumerate(self.waypoints):
            ...
            uav2_find = self.findAttachmentPoint(droneId1)
