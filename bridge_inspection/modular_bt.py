...
class BridgeInspectionPhase1:
    ...
    def build_tree_from_dict(self, tree_in_dict_form):
        if not tree_in_dict_form:
            return None
        
        node_type = tree_in_dict_form['type'].replace('-', '').lower()
        
        # Composite Nodes
        if node_type == 'selector':
            selector = py_trees.composites.Selector(
                name=tree_in_dict_form['type'],
                memory=True
            )
            for child in tree_in_dict_form.get('children', []):
                child_node = self.build_tree_from_dict(child)
                if child_node:
                    selector.add_child(child_node)
            return selector
            ...
            for child in tree_in_dict_form.get('children', []):
                child_node = self.build_tree_from_dict(child)
                if child_node:
                    parallel.add_child(child_node)
            return parallel
        
        # Leaf Nodes (Custom Behaviours)
        elif node_type == 'takeoff1':
            return self.takeOff(1)
        elif node_type == 'takeoff2':
            return self.takeOff(2)
        elif node_type == 'acquiretarget':
            return self.acquireTarget(1)
        elif node_type == 'approachtarget':
            return self.approachTarget(2)
        elif node_type == 'cleansurface':
            return self.cleanSurface(1)
        elif node_type == 'attachsensor':
            return self.attachSensor(2)
        elif node_type == 'recovery1':
            return self.createRecoveryTree(1)
        elif node_type == 'recovery2':
            return self.createRecoveryTree(2)

        else:
            logging.error(f"Unknown node type: {tree_in_dict_form['type']}")
            return None
...
def main():
    ...
    swarm = SwarmConductor(
        ...
        # Dictionary representation of Phase 1 mission
    phase1_dict = {
        "type": "parallel",
        "children": [
            {
    ...
    try:
        ...
        root_node = phase1.build_tree_from_dict(phase1_dict)
